...
if JOB.gc
    ...
    %% obtain in the standard Granger causal way, using mvgc
    measure_names = {'gc'};
    results = calc_measures(A,SIG,measure_names,1);
    ...
    [ratio_S ratio] = ratio_of_dets(S_f, S_r, SIG, SIG_r);
    ...
    pc1=SIG(1,1)-SIG(1,2)/SIG(2,2)*SIG(2,1);
    pc2=SIG(2,2)-SIG(1,2)/SIG(1,1)*SIG(2,1);
...
%     
    ...
    % freq domain plot
    ph1to2 = plot(freqs,results.sdecomp_GC_x1_to_x2,freqs,real(squeeze(t_GC_spct(2,1,:))),'--r');
    set(ph1to2, 'linewidth', 3)
    
    %time domain quantities
    title(sprintf('GC x1 to x2. Ours: %d, MVGC: %d',t_GC(2,1), results.GC_x1_to_x2));
    ...
    %other direction
    subplot(1,2,2)
    ph2to1 = plot(freqs,sdecomp_GC_x2_to_x1,freqs,real(squeeze(results.t_GC_spct(1,2,:))),'--r');
   ...
   %we have GC in %(x2 to x1)
    ...
    H_tilde11=squeeze(H(1,1,:)+SIG(1,2)/SIG(1,1)*H(1,2,:));
    H_tilde22=squeeze(H(2,2,:)+SIG(1,2)/SIG(2,2)*H(2,1,:));
    
    GC_x2_to_x1_tf =  squeeze(S_f(1,1,:)) ./ (H_tilde11 .* conj(H_tilde11) .* SIG(1,1));
    GC_x1_to_x2_tf =  squeeze(S_f(2,2,:)) ./ (H_tilde22 .* conj(H_tilde22) .* SIG(2,2));
    ...
    % check denom equiv
    plot(real(squeeze(H_tilde11 .* conj(H_tilde11) .* SIG(1,1)) - squeeze(Ps2_f)))
    
    %check partialization equivalence
    clf
    plot(real(squeeze(H_tilde11 .* conj(H_tilde11) .* SIG(1,1))) - real(squeeze(Ps1_f)))
%     plot(real(squeeze(H_tilde11 .* conj(H_tilde11) .* SIG_f(1,1))) - real(squeeze(Ps1_f)))
%     plot(real(squeeze(H_tilde11 .* conj(H_tilde11) .* SIG_f(1,1))) - real(squeeze(Ps1_f)))

    
    %check ratio
        plot(real(GC_x2_to_x1_tf - squeeze(Ps2_r./Ps2_f)))
...
% estimate it
    ...
    t_stoch_info_2 = log ( univar_SIG1*univar_SIG2  / det (SIG) );
...
%estimate
    ...
    log_E22_1 = tmpS(2,2); 
    log_E11_1 = tmpS(1,1);
    t_stoch_info_3=log_E22_1+log_E11_1-log(det (SIG));
...
% compare against instantatneou caus, time domain
    t_inst_info2=log(SIG(1,1).*SIG(2,2)/det(SIG));
    ...
    t_inst_info_spct23 = log( H_tilde11.*conj(H_tilde11).*SIG(1,1) .* SIG(2,2).*H_tilde22.*conj(H_tilde22) ./ det_S_f );
